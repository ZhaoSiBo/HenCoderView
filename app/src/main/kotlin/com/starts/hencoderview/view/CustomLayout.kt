...
abstract class CustomLayout : ViewGroup {
    ...
    protected data class Dimension(val width: Int, val height: Int)

    protected abstract fun onMeasureChildren(
    ...
    fun View.horizontalCenterX(child: View): Int {
        return (measuredWidth - child.measuredWidth) / 2
    }
    ...
    fun View.verticalCenterTop(child: View): Int {
        return (measuredHeight - child.measuredHeight) / 2
    }
}
...
fun View?.performHapticFeedbackSafely() {
    try {
        ...
        Timber.tag("CustomLayout").e(t.toString())
    }
}
