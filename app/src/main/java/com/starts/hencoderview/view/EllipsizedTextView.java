...
public class EllipsizedTextView extends TextView {
    ...
    private SpannableStringBuilder mUnFoldText;
    ...
    public EllipsizedTextView(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
        ...
        TypedArray a = context.obtainStyledAttributes(attrs, R.styleable.EllipsizedTextView);
        String unFoldString = a.getString(R.styleable.EllipsizedTextView_unFoldText );
        if(TextUtils.isEmpty(unFoldString)){
            unFoldString = DEFAULT_UNFOLD_TEXT;
        }
        CharSequence temp = unFoldString;
        mUnFoldText = new SpannableStringBuilder(new SpannableString(temp));
        mFoldLine = a.getInt(R.styleable.EllipsizedTextView_foldLine, getMaxLines());
        if (mFoldLine < 1) {
            mFoldLine = 1;
        }
        mTailColor = a.getColor(R.styleable.EllipsizedTextView_tailTextColor, DEFAULT_TAIL_TEXT_COLOR);
        a.recycle();
    }
    ...
    public void  setUnFoldText(SpannableStringBuilder span){
        mUnFoldText = null;
        mUnFoldText = span;
        resetText();
    }
